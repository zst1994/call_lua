...
function model:change_IP(content_user,content_country)--换IP
	while true do
		::change_ip::
		mSleep(500)
		status_resp, header_resp,body_resp = ts.httpGet("http://www.luminati-cn.net/changeip/8.210.91.214/"..content_user)
		if status_resp == 200 then
			tmp = json.decode(body_resp)
			if tmp.state == "ok" then
				toast("切换成功",1)
				mSleep(1000)
				break
			else
...
function model:wechat(move_type,operator,login_times,content_user,content_country,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,country_len,login_type,addBlack,diff_user,ran_pass,ddwGet,airplaneStatus)
	account_len = 0
	old_mess_yzm = ""
...
function model:main()
	...
	while true do
		...
		self:clear_App()
		self:wechat(move_type,operator,login_times,content_user,content_country,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,country_len,login_type,addBlack,diff_user,ran_pass,ddwGet,airplaneStatus)
		mSleep(1000)
	end
end
