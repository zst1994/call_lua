...
function model:wechat(ksUrl,move_type,operator,login_times,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,country_len,login_type,addBlack,diff_user,ran_pass,airplaneStatus,tableKey,tmBack,tmFail,waitYZM)
	account_len = 0
	failTM = 0
	old_mess_yzm = ""
			...
			elseif vpn_stauts == "8" then
				...
				if code == 200 then
					...
					elseif tmp.code == 908 then
						...
						mSleep(2000)
						get_time = get_time + 1
						if get_time > tonumber(waitYZM) then
							if country_id == "886" then
								mSleep(math.random(2000, 3000))
								randomsTap(372,  749, 3)
								mSleep(math.random(1000, 1500))
...
function model:main()
	...
	ret, move_type, login_times, content_type, vpn_stauts, kn_country, kn_id, countryId, country_len, login_type, fz_terrace, nickName, password, operator, addBlack, diff_user, ran_pass, airplaneStatus, tableKey, tmBack, tmFail, waitYZM = showUI(MyJsonString)
	if ret == 0 then
		dialog("取消运行脚本", 3)
		luaExit()
	end
	...
	while true do
		...
		self:clear_App()
		self:wechat(ksUrl,move_type,operator,login_times,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,country_len,login_type,addBlack,diff_user,ran_pass,airplaneStatus,tableKey,tmBack,tmFail,waitYZM)
		mSleep(1000)
	end
end
