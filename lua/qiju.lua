...
function model:ewm(phone,ewm_url,fz_terrace,content_type,load_ewm_bool,base_six_four)
	if fz_terrace == "0" then
		...
		::put_work::
		header_send = {
			token = "D2DC51BBFD13C920A9CF255A73306F20A1B678C5151C9245F98E0BEBFAA6D973"
		}
	...
	elseif fz_terrace == "1" then
		...
		::put_work::
		header_send = {}
		body_send = string.format("userKey=%s&"..qrCode.."=%s&phone=%s&provinceId=%s","1A73647DDB5FEBCAD2EF8AAFE651061A",ewm_url,phone,"210000")
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
	...
	elseif fz_terrace == "2" then
		...
		header_send = {}
		body_send = string.format("userKey=%s&"..qrCode.."=%s&phone=%s&provinceId=%s","A8F944F226B64F6DD2D53A7FBC73A338",ewm_url,phone,"210000")
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
		status_resp, header_resp, body_resp  = ts.httpPost("http://card-api.mali126.com/api/order/submit", header_send, body_send, true)
		toast(body_resp,1)
		if status_resp == 200 then
	...
	elseif fz_terrace == "3" then
		::put_work::
		header_send = {
		}
		body_send = string.format("userKey=%s&qrCodeUrl=%s&phone=%s","2b3e8f5f7448445f85e8f1f48d36f72c",ewm_url,phone)
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
	...
	while (true) do
		...
		if x~=-1 and y~=-1 then
			...
			if fz_terrace == "0" then
				status = 2
			elseif fz_terrace == "1" or fz_terrace == "2"  then
				status = 1
			elseif fz_terrace == "3" then
				status = 3
			end
		...
		mSleep(math.random(500,700))
		x,y = findMultiColorInRegionFuzzy( 0x007aff, "3|14|0x007aff,14|-5|0x007aff,18|17|0x007aff,11|7|0x007aff,23|7|0x007aff", 90, 0, 0, 749, 1333)
		if x~=-1 and y~=-1 then
			mSleep(math.random(3500, 5700))
			randomsTap(x, y,5)
	...
	elseif fz_terrace == "1" then
		::push_work::
		header_send = {}
		body_send = string.format("userKey=%s&orderId=%s&status=%s","1A73647DDB5FEBCAD2EF8AAFE651061A",taskId,status)
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
	...
	elseif fz_terrace == "2" then
		::push_work::
		header_send = {}
		body_send = string.format("userKey=%s&orderId=%s&status=%s","A8F944F226B64F6DD2D53A7FBC73A338",taskId,status)
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
		status_resp, header_resp, body_resp  = ts.httpPost("http://card-api.mali126.com/api/order/mark", header_send, body_send, true)
		if status_resp == 200 then
			local tmp = json.decode(body_resp)
	...
	elseif fz_terrace == "3" then
		...
		body_send = string.format("userKey=%s&orderId=%s&status=%s", "2b3e8f5f7448445f85e8f1f48d36f72c", orderId, status)
		ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
...
function model:wechat(move_type,operator,login_times,content_user,content_country,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,login_type,ran_pass)
	account_len = 0
	old_mess_yzm = ""
	...
	if vpn_stauts == "1" then
		...
		local http = require("szocket.http")
		local res, code = http.request("http://47.56.103.47/api.php?action=Login&user=xiaoyun516&pwd=225916abc")
	...
	elseif vpn_stauts == "3" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("http://52yzm.top/handler/api.php?apikey=95d43200c7bd7814d5f257edec3fee21&action=getnumber&country="..countryId.."&service=67")
		mSleep(500)
		if code == 200 then
	...
	elseif vpn_stauts == "4" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("https://smsregs.ru/api/v1/get_number?token=xiaoyun516:ACD38AE0B3FFB516E01A&country="..countryId.."&service=wechat")
		mSleep(500)
		if code == 200 then
			tmp = json.decode(res)
	...
	elseif vpn_stauts == "5" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("http://172.96.210.124:9192/api/mobile?token=292d14172e0f0bd5ad5e96dac30f482d4b7fa04f&src=tl&app=WeChat&username=taishan18")
		mSleep(500)
		if code == 200 then
			tmp = json.decode(res)
	...
	elseif vpn_stauts == "6" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("http://39.98.56.33/get_data")
		mSleep(500)
		if code == 200 then
			tmp = json.decode(res)
	...
	elseif vpn_stauts == "7" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("http://simsms.org/priemnik.php?metod=get_number&country="..countryId.."&service=opt67&apikey=QWX6PApNRrGGMdWmwRvM6NnmI9XWVa")
		if code == 200 then
			tmp = json.decode(res)
	...
	elseif vpn_stauts == "8" then
		...
		local sz = require("sz")        --登陆
		local http = require("szocket.http")
		local res, code = http.request("https://sms-activate.ru/stubs/handler_api.php?api_key=289e875d4d437B987e0e12161508363e&action=getNumber&service=wb&country="..countryId)
		nLog(res)
		if code == 200 then
			tmp = strSplit(res, ":")
			if #tmp > 1 then
			...
			toast("获取手机号码失败，重新获取:"..code,1)
			mSleep(30000)
			goto get_phone
		end
	else
		dialog("平台选错了，脚本重新运行请重新勾选", 5)
		lua_restart()
	end
	
	if vpn_stauts == "1" or vpn_stauts == "2" or vpn_stauts == "3" or vpn_stauts == "5" or vpn_stauts == "6" or vpn_stauts == "8" then
	...
	if vpn_stauts == "1" or vpn_stauts == "3" or vpn_stauts == "4" or vpn_stauts == "6" or vpn_stauts == "7" then
		phone = telphone
	...
	elseif vpn_stauts == "2" then
		...
		phone = string.sub(tel_phone, lens+2, #tel_phone)
	end
	...
	writeFileString(userPath().."/res/phonePass.txt",country_id..phone.."----"..password,"a",1)
	
	cheack_bool = true
	set_vpn = false
	::tiaoma::
	if set_vpn then
		if content_type == "0" or content_type == "3" then
			self:vpn()
		end
	end
	...
	if cheack_bool then
		...
		elseif login_type == "1" or login_type == "3" then
			...
			if login_type == "1" then
				...
				while (true) do
					...
					if getColor(132,  766) == 0x000000 then
						...
						if fz_terrace == "0" or fz_terrace == "3" then
							::ewm_go::
			...
			if ewm_url_bool then
				toast("准备发布任务",1)
				mSleep(1000)
				ewm_bool = self:ewm(phone,ewm_url,fz_terrace,content_type,load_ewm_bool,base_six_four,login_type)
				if ewm_bool then
					mSleep(500)
					toast("辅助成功",1)
					mSleep(500)
	...
	::hk_again::
	if move_type == "0" then
		if login_type == "0" or login_type == "2" or login_type == "3" or login_type == "4" then 
			--滑块
			::get_pic::
			while (true) do
				mSleep(math.random(500, 700))
		...
		while (true) do
			...
			if getColor(132,  766) == 0x000000 then
				...
				if fz_terrace == "0" then
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://sj.golddak.com/api/surround/v1/status/"..phone.."?token=D2DC51BBFD13C920A9CF255A73306F20A1B678C5151C9245F98E0BEBFAA6D973")
					mSleep(500)
					if code == 200 then
						tmp = json.decode(res)
						if tmp.message == "success" then
				...
				elseif fz_terrace == "1" then
					...
					body_send = string.format("key=%s&tel=%s&area=%s","1A73647DDB5FEBCAD2EF8AAFE651061A",phone,country_id)
					ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://card-api.mali126.com/api/order/telquery?key=1A73647DDB5FEBCAD2EF8AAFE651061A&orderId="..orderId)
					mSleep(500)
					if code == 200 then
						tmp = json.decode(res)
						if tmp.success then
				...
				elseif fz_terrace == "2" then
					...
					body_send = string.format("key=%s&tel=%s&area=%s","A8F944F226B64F6DD2D53A7FBC73A338",phone,country_id)
					ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
					status_resp, header_resp, body_resp  = ts.httpPost("http://card-api.mali126.com/api/order/telsubmit", header_send, body_send, true)
					mSleep(1000)
					nLog(body_resp)
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://card-api.mali126.com/api/order/telquery?key=A8F944F226B64F6DD2D53A7FBC73A338&orderId="..orderId)
					mSleep(500)
					if code == 200 then
						tmp = json.decode(res)
						if tmp.success then
	...
	while true do
		...
		if x~=-1 and y~=-1 then
			if vpn_stauts == "2" then
				...
				local sz = require("sz")        --登陆
				local http = require("szocket.http")
				local res, code = http.request("http://api5.caugu.com/yhapi.ashx?act=sendCode&token="..phone_token.."&pid="..pid.."&receiver="..phone_name.."&smscontent="..content_num)
				mSleep(500)
				if code == 200 then
					data = strSplit(res, "|")
					if data[1] == "1" then
	...
	if mess_bool then
		if get_ms_code then
			...
			elseif vpn_stauts == "3" then
				...
				local sz = require("sz")        --登陆
				local http = require("szocket.http")
				local res, code = http.request("http://52yzm.top/handler/api.php?apikey=95d43200c7bd7814d5f257edec3fee21&action=getsms&country="..country_id.."&service=67&id="..pid)
				mSleep(500)
				if code == 200 then
					data = strSplit(res, ":")
					if reTxtUtf8(data[1]) == "OK" then
					...
					elseif reTxtUtf8(data[1]) == "BAD" then
						...
						if get_time > 20 then
							...
							local sz = require("sz")        --登陆
							local http = require("szocket.http")
							local res, code = http.request("http://52yzm.top/handler/api.php?apikey=95d43200c7bd7814d5f257edec3fee21&action=ban&service=67&id="..pid)
							mSleep(500)
							if code == 200 then
								data = strSplit(res, ":")
								if reTxtUtf8(data[1]) == "OK" then
			...
			elseif vpn_stauts == "4" then
				...
				local sz = require("sz")        --登陆
				local http = require("szocket.http")
				local res, code = http.request("https://smsregs.ru/api/v1/get_sms?token=xiaoyun516:ACD38AE0B3FFB516E01A&request_id="..requestId)
				mSleep(500)
				if code == 200 then
					tmp = json.decode(res)
			...
			elseif vpn_stauts == "5" then
				...
				local sz = require("sz")        --登陆
				local http = require("szocket.http")
				local res, code = http.request("http://172.96.210.124:9192/api/code?token=292d14172e0f0bd5ad5e96dac30f482d4b7fa04f&src=tl&app=WeChat&username=taishan18&mobile="..telphone)
				mSleep(500)
				if code == 200 then
					tmp = json.decode(res)
					if type(tmp.data) ~= "nil" and #(tmp.data:atrim()) > 0 then
						mess_yzm = tmp.data
					else
						toast("暂无短信"..get_time,1)
						mSleep(5000)
			...
			elseif vpn_stauts == "6" then
				...
				if yzm_bool then
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://39.98.56.33/update_data?id="..result_id)
					mSleep(500)
					if code == 200 then
						tmp = json.decode(res)
			...
			elseif vpn_stauts == "7" then
				while true do
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://simsms.org/priemnik.php?metod=get_sms&country="..countryId.."&service=opt67&apikey=QWX6PApNRrGGMdWmwRvM6NnmI9XWVa&id="..pay_id)
					if code == 200 then
						tmp = json.decode(res)
						if tmp.response == "1" then
							toast(tmp.sms, 1)
					...
					if get_time > 5 then
						...
						local sz = require("sz")        --登陆
						local http = require("szocket.http")
						local res, code = http.request("http://simsms.org/priemnik.php?metod=ban&service=opt67&apikey=QWX6PApNRrGGMdWmwRvM6NnmI9XWVa&id="..pay_id)
						if code == 200 then
							tmp = json.decode(res)
							if tmp.response == "1" then
								toast("手机号码拉黑成功", 1)
			...
			elseif vpn_stauts == "8" then
				...
				local sz = require("sz")        --登陆
				local http = require("szocket.http")
				local res, code = http.request("https://sms-activate.ru/stubs/handler_api.php?api_key=289e875d4d437B987e0e12161508363e&action=getStatus&id="..phone_id)
				mSleep(500)
				if code == 200 then
					tmp = strSplit(res, ":")
					if #tmp > 1 then
						mess_yzm = tmp[2]
					elseif tmp[1] == "STATUS_WAIT_CODE" then
						toast(tmp[1]..":"..get_time,1)
						mSleep(5000)
						get_time = get_time + 1
						if get_time > 15 then
							mSleep(500)
			...
			for i = 1, 6 do
				...
				if x~=-1 and y~=-1 then
					...
					mSleep(math.random(1000, 1500))
					randomsTap(368, 1039,5)
					mSleep(math.random(10000, 16000))
					setVPNEnable(false)
					old_mess_yzm = mess_yzm
					get_time = 1
					restart_time = 0
					goto get_new_mess
				end
		...
		if data_six_two or error_wechat then
			...
			local sz = require("sz")       
			local http = require("szocket.http")
			local res, code = http.request("http://39.98.56.33/import_data?phone="..user.."&password="..password.."&token="..six_data.."&is_normal="..is_normal.."&operator="..operator.."&link="..urlEncoder(api).."&time="..sj)
			if code == 200 then
				tmp = json.decode(res)
...
function model:main()
	...
	MyTable = {
		...
		views = {
			...
			{
				["type"] = "RadioGroup",                    
				["list"] = "无,卡农注册,奥迪,52,俄罗斯1,东帝汶,服务器取号,天真3api,天真2",
				["select"] = "0",  
				["countperline"] = "4",
			},
			{
				["type"] = "Label",
				["text"] = "卡农注册/奥迪/52/东帝汶/服务器取号/天真2的区号",
				["size"] = 15,
				["align"] = "center",
				["color"] = "255,0,255",
			},
			{
				["type"] = "Edit",        
				["prompt"] = "请输入卡农注册/奥迪/52/东帝汶/服务器取号/天真2的区号",
				["text"] = "默认值",       
			},
			{
				["type"] = "Label",
			...
			{
				["type"] = "Label",
				["text"] = "设置52/俄罗斯1/天真3api/天真2的国家代码",
				["size"] = 20,
				["align"] = "center",
				["color"] = "0,0,255",
			},
			...
			{
				["type"] = "RadioGroup",                    
				["list"] = "跳马注册,二维码辅助注册,手机号辅助注册,滑块链接注册,手动辅助",
				["select"] = "0",  
				["countperline"] = "3",
			},
			{
	...
	ret, move_type, login_times, content_type, content_user, content_country, vpn_stauts, kn_country, kn_id, countryId, login_type, fz_terrace, nickName, password, operator, ran_pass = showUI(MyJsonString)
	if ret == 0 then
		dialog("取消运行脚本", 3)
		luaExit()
	end
	...
	while true do
		...
		if vpn_stauts == "2" then
			...
			local sz = require("sz");
			local http = require("szocket.http")
			local res, code = http.request("http://www.3cpt.com/yhapi.ashx?act=login&ApiName=xiaoyun51651601&PassWord=225916abc")
			if code == 200 then
				data = strSplit(res,"|")
				if data[1] == "1" then
					phone_token = data[2]
		...
		self:wechat(move_type,operator,login_times,content_user,content_country,content_type,vpn_stauts,phone_token,kn_country,kn_id,countryId,nickName,password,login_type,ran_pass)
		mSleep(1000)
	end
end
