...
function model:ewm(phone,country_id,ewm_url,fz_terrace,load_ewm_bool,base_six_four)
	...
	elseif fz_terrace == "1" then
		...
		body_send = string.format("userKey=%s&"..qrCode.."=%s&phone=%s&provinceId=%s","B4287CB20B7F66D376991F3D17029838",ewm_url,phone,"210000")
	...
	elseif fz_terrace == "1" then
		...
		body_send = string.format("userKey=%s&orderId=%s&status=%s","B4287CB20B7F66D376991F3D17029838",taskId,status)
		...
		while (true) do
			...
			if getColor(132,  766) == 0x000000 then
				...
				elseif fz_terrace == "1" then
					...
					body_send = string.format("key=%s&tel=%s&area=%s","B4287CB20B7F66D376991F3D17029838",phone,country_id)
					ts.setHttpsTimeOut(60)--安卓不支持设置超时时间 
					...
					local sz = require("sz")        --登陆
					local http = require("szocket.http")
					local res, code = http.request("http://card-api.mali126.com/api/order/telquery?key=B4287CB20B7F66D376991F3D17029838&orderId="..orderId)
					mSleep(500)
					if code == 200 then
						tmp = json.decode(res)
						if tmp.success then
...
function model:main()
	...
	ret, move_type, login_times, content_type, content_user, content_country, vpn_stauts, kn_country, kn_id, countryId, country_len, login_type, fz_terrace, nickName, password, operator, addBlack, diff_user, ran_pass = showUI(MyJsonString)
	if ret == 0 then
		dialog("取消运行脚本", 3)
		luaExit()
	end
